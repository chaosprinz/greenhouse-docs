...
const validateBody = async(body: any) => {
    if (!body.temperature) return {
        valid: false,
        msg: "no temperature given",
    };

    if (!Number.isInteger(body.temperature)) return {
        valid: false,
        msg: "temperature is not an int",
    };

    if (!body.humidity) return {
        valid: false,
        msg: "no humidity given",
    };

    if (!Number.isInteger(body.humidity)) return {
        valid: false,
        msg: "humidity is not an int",
    };

    if (!body.growId) return {
        valdid: false,
        msg: "no growId given",
    };

    if (!Number.isInteger(body.growId)) return {
        valid: false,
